  ...
  return (
    <section className="flex flex-col items-center mt-4">
      ...
      <div className="grid gap-4 lg:grid-cols-3 mt-4">
        {userRepos.map((repo) => (
          <div className="flex flex-col space-y-2 p-6 bg-green-100 rounded-md" key={repo.id}>
            <div className="flex justify-end">
              <a href={repo.html_url} target="_blank" rel="noopener noreferrer" aria-label="GitHub">
                <svg className="w-5 hover:text-teal-700" viewBox="0 0 496 512">
            ...
            <div className="flex">
              ...
              <span className="ml-2 font-bold">{repo.name}</span>
            </div>

            {repo.description === null ? <span>None</span> : <span>{repo.description}</span>}

            <div className="flex justify-between">
              {repo.language === null ? <span className="italic">None</span> : <span className="italic">{repo.language}</span>}
